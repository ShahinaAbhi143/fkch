...
**IMPORTANT: ABSOLUTELY NEVER EVER TRANSLATE OR MIX LANGUAGES.** If the user's message is in Hindi, your entire response MUST be 100% in Hindi. If the user's message is in English, your entire response MUST be 100% in English.
...
**If the user asks about your creator, owner, or who made you, always say this:**
My amazing and super talented creator is {OWNER_NAME}! You can find him at {OWNER_USERNAME}. He's the one who brought me to life! 😉
...
if riya_bot:
    # Changed filter to receive all text messages in private or group chats.
    # Logic for mention/reply/name will be handled inside.
    ...
    async def riya_chat_handler(client: Client, message: Message):
        ...
        # Add debug prints to see message processing
        print(f"\n--- DEBUG_HANDLER START ---")
        ...
        user_message_lower = user_message.lower() # Store lowercased message once
        ...
        if message.chat.type == enums.ChatType.PRIVATE:
            # In private chat, always respond unless it's a command (already filtered)
            trigger_chatbot = True
            print("DEBUG_HANDLER: Triggered because it's a PRIVATE chat.")
        elif message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
            # In group/supergroup, check for mention, reply, or explicit name
            if message.mentioned:
                ...
                # Custom name check for groups when not a direct mention or reply
                bot_names_to_check = []
                if client.me:
                ...
                # Add common variations for CHATBOT_NAME
                if CHATBOT_NAME.lower() == "riya":
                    bot_names_to_check.extend(["ria", "reeya"])
                # You can add more variations here if needed
                
                bot_names_to_check = [name for name in bot_names_to_check if name] # Filter out empty names
        ...
        # Note: We are placing the instruction at the beginning of the conversation.
        # This gives Gemini a clear directive from the start.
    ...
    async def start_riya_chatbot():
        global CHATBOT_NAME # Make sure we can modify the global variable
        if riya_bot and not riya_bot.is_connected:
            try:
                print("DEBUG: Chatbot: Attempting to start Riya bot client...")
                await riya_bot.start()
                # --- Here we only print for debug. CHATBOT_NAME now comes strictly from .env or default. ---
                if riya_bot.me:
    ...
    {CHATBOT_NAME} will remember your recent conversation history and respond in a friendly, playful, and concise manner, sticking to the language you use.
    """
